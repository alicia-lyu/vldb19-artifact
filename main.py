...
class DataPerConfig:
    ...
    """
    def __init__(self, scale: int, bgw_pct: int, dram_size: int, df: pd.DataFrame, parent_dir: str):
        self.scale = scale
    ...
    def optimal_skip(self, tx_df: pd.DataFrame, tx: str, y_label: str):
        ...
        print(f'optimal skip for {tx} in {self.directory} with dram={self.dram_size}, scale={self.scale}, bgw_pct={self.bgw_pct}: {optimal_skip}')
        return optimal_skip
    ...
    def group_and_anonymize(self):
        ...
        for (tx), tx_df in point_per_tx:
            ...
            if tx.startswith('join'):
                anonymized_name = f"Q{tx_count}-join"
            elif tx.startswith('mixed'):
                anonymized_name = f"Q{tx_count}-mixed"
            else:
                suffix = 'update'
                anonymized_name = "Update"
            txs[anonymized_name] = tx_df.groupby('method').mean()[y_label].round(0).astype(int)
    ...
    def plot_queries(self, detailed: bool):
        ...
        else: # exclude mixed and update
            df_copy = df_copy[~df_copy.index.str.contains('mixed')]
            df_copy = df_copy[~df_copy.index.str.contains('Update')]

        width = len(df_copy.columns) * 1.5 + 1.5
...
class BenchmarkAnalyzer:
    ...
    def __init__(self, directory: str):
        ...
        self.elapsed_df['DRAM (GiB)'].replace(0.09, 0.1, inplace=True)  # Normalize 0.09 to 0.1 (0.01 for hash table)
        self.tput_df = pd.read_csv(self.dir / 'TPut.csv')
        self.tput_df['DRAM (GiB)'].replace(0.09, 0.1, inplace=True)  # Normalize 0.09 to 0.1 (0.01 for hash table)
        self.dfs_per_config = {}
    ...
    def update_of_all_configs(self):
        ...
        for config, dpc_list in self.dfs_per_config.items():
            ...
            dbtoaster_col = {'DBToaster': int(dbtoaster_update_df['update_time_us'])}
            update_by_dram = pd.concat([update_by_dram, pd.DataFrame(dbtoaster_col, index=[closest_dram])], axis=1)
    ...
    def _plot_update(self, df: pd.DataFrame, scale_value: int, bgw_pct: int, unit: str, summary: bool):
        ...
        inf_df.plot(kind='bar', ylabel=f'Update time ({unit})', legend=True, ax=ax, xlabel='', table=(not summary), logy=False, color=[COLORS.get(col, 'C5') for col in inf_df.columns])
        ymax = ax.get_ylim()[1]
        ...
        for i, col in enumerate(replaced_df.columns):
            ...
            col_df.plot(kind='bar', ylabel=f'Update time ({unit})', ax=ax, xlabel='', logy=False, clip_on=True, legend=False, hatch='//', color='none', edgecolor=COLORS.get(col, 'C5'))
        if not summary:
            ax.set_xticks([], [])
